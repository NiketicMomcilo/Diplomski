...
public class EnterInputDataEncryption implements Initializable {
    ...
    boolean errorCheck(){
        String key = this.encryptionKeyTextField.getText().replace(" ", "");
        String plaintext = this.plaintextTextField.getText().replace(" ", "");
        ...
        if(plaintext.length() != 32){
            errorLabel.setTextFill(Color.RED);
            errorLabel.setText("Plaintext must have 32 characters not including spaces");
            return false;
        }
        if(!plaintext.matches("^[0-9A-F]+$")){
            errorLabel.setTextFill(Color.RED);
            errorLabel.setText("Plaintext must be hex number consisting only of 0-9, A-F");
            return false;
        }
        return true;
    }
