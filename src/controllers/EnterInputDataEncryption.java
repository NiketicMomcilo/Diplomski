package controllers;

import javafx.event.ActionEvent;
import javafx.fxml.FXMLLoader;
import javafx.fxml.Initializable;
import javafx.scene.Parent;
import javafx.scene.Scene;
import javafx.scene.control.Button;
import javafx.scene.control.TextField;

import java.io.IOException;
import java.net.URL;
import java.util.ResourceBundle;
import sample.*;

public class EnterInputDataEncryption implements Initializable {

    public TextField encryptionKeyTextField;
    public TextField sArrayTextField;
    public TextField lArrayTextField;
    public TextField plaintextKeyTextField;
    public Button nextButton;

    @Override
    public void initialize ( URL url, ResourceBundle resourceBundle ) {

    }


    public void setDefaultEncryptionData ( ActionEvent actionEvent ) {

        this.encryptionKeyTextField.setText("01 23 45 67 89 ab cd ef 01 12 23 34 45 56 67 78 89 9a ab bc cd de ef f0 10 32 54 76 98 ba dc fe");
        this.plaintextKeyTextField.setText("45 46 47 48 c8 24 18 16 f0 d7 e4 89 20 ad 16 a1 ");

    }

    public void generateKey ( ActionEvent actionEvent ) {
        byte[] key = RC6.hexStringToByteArray(encryptionKeyTextField.getText().toString().replace(" ",""));

        new RC6().KeySchedule(key);

        StringBuilder sb = new StringBuilder();

        for ( int i = 0 ; i < RC6.S.length ; i++ ) {
            sb.append(RC6.S[i] + " ");
        }
        this.sArrayTextField.setText(sb.toString());

        sb = new StringBuilder();

        for ( int i = 0 ; i < RC6.L.length ; i++ ) {
            sb.append(RC6.L[i] + " ");
        }
        this.lArrayTextField.setText(sb.toString());




    }

    public void nextScene ( ActionEvent actionEvent ) throws IOException {

        Parent root = FXMLLoader.load(getClass().getResource("../forms/FirstRound.fxml"));

        Main.primaryStage.setTitle("FirstRound");
        Main.primaryStage.setScene(new Scene(root, 773, 625));

    }
}
